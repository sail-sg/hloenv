...
PYBIND11_MODULE(py_hlo_env, m) {
  ...
  py::class_<PyHloEnv::EvaluationResult>(
      m, "EvaluationResult",
      "A structure containing the duration and output of the HloModule "
      "evaluation.")
      .def_readonly(
          "durations", &PyHloEnv::EvaluationResult::durations,
          "The default duration in nanoseconds. This returns the execution "
          "duration as measured within the Tensorflow evaluation code, "
          "starting from the point when the executable has been enqueued on "
          "the compute stream till the completion of the executable.")
      .def_readonly("compute_durations",
                    &PyHloEnv::EvaluationResult::compute_durations,
                    "The duration in nanoseconds of the computation, without "
                    "data transfer, as measured on the device.")
      .def_readonly(
          "full_durations", &PyHloEnv::EvaluationResult::full_durations,
          "The full duration in nanoseconds as measured within "
          "HloEnv.evaluate(). This captures the entire execution process "
          "including processes such as enqueueing the computation on the "
          "compute stream, and is hence more subject to timing noise.")
  ...
  py::class_<AltHloModule, std::shared_ptr<AltHloModule>>(
      m, "HloModule",
      "The class representing an XLA HloModule. Each HloModule can be loaded "
      "into the :class:`HloEnv`, where we can evaluate it, obtain its hash, or "
      "run specific :class:`Pass` and :class:`Pipeline` on it ")
...
Args:
    ...
    format (str, optional): The format of the input. Can be either "path" for a filepath to a Hlo text file, or "txt" for the raw Hlo text string. Defaults to "path".
      ...
      .def("to_string", &AltHloModule::ToString,
           "Converts the HloModule to a string representation. This string "
           "representation can also used to initialize a new HloEnv or loaded "
           "into an existing one.")
      .def_property_readonly("config", &AltHloModule::config,
                             "The config options of the HloModule")
      .def("hash", &AltHloModule::Hash,
           "The DAGHash of the HloModule. This DAGHash is a custom hash "
           "implementation that differs from Tensorflow's existing HloModule "
           "hash implementation to better account for the structure and "
           "parameters of the Hlo Instructions.")
      ...
      .def("is_bef_enabled", &AltHloModule::IsBefEnabled,
           "Returns whethe Binary Executable Format (BEF) is enabled for the "
           "executable.")
      ...
      .def("cost_analysis", &AltHloModule::CostAnalysis,
           "Returns a dictionary containing the flops, transcendentals, bytes "
           "accessed and optimal seconds of the Hlo Module.")
      .def("clone", &AltHloModule::Clone, "Clones the HloModule.");

  py::class_<PyHloEnv>(
      m, "HloEnv",
      "The class representing the HloEnv. Each HloEnv instance can be loaded "
      "with a single HloModule, and used to run Passes/Pipelines on that "
      "module, as well as extract features, evaluate and obtain the graph "
      "features for that HloModule.")
      .def(py::init<const std::string&, const std::string&>(),
           R"hloenvdoc(
Creates a :class:`HloEnv` and loads in a HloModule from a specified filepath.
...
Args:
    ...
    platform (str): The platform we wish to run the HloModule on. Currently only 'gpu' is supported
      ...
      .def("export_hlo_to_str", &PyHloEnv::ExportHloModuleToStr,
           "Exports the currently loaded :class:`HloModule` into its text "
           "representation. This text can be loaded by the HloEnv.")
      ...
      .def("get_hlo_graph", &PyHloEnv::GetHloGraph, py::arg("debug") = false,
           "Converts the :class:`HloModule` into a :class:'HloGraph' object "
      ...
      .def("prepare_for_eval", &PyHloEnv::PrepareForEvaluation,
           "Prepare the HloModule for IR emitting and evaluation. This step is "
           "automatically run during HloEnv.evaluate, unless the "
           "do_not_prep_for_eval parameter is set to True, hence in most cases "
           "you will not have to run this function.")
